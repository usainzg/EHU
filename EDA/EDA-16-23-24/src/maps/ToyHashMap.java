...
public class ToyHashMap<K, V> {
	...
	public V remove(K key) {
		...
		MapEntry<K, V> currentEntry = entries[hash];
		V valueRet = get(key);
		
		while (currentEntry != null) {
			if (currentEntry.key.equals(key)) {
				if (prevEntry == null) {
					entries[hash] = entries[hash].next; // primer elemento
				} else {
	...
	public Object[] getAllKeys() {
		MapEntry<K, V>[] entriesCopy = entries;
		Object[] arrRet = new Object[size];
		int i = 0;
		
		for (MapEntry<K, V> entry: entriesCopy) {
			if (entry != null) {
				while (entry != null) {
					arrRet[i] = entry.key;
					i++;
