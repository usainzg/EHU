...
public class LinkedOrdList<T extends Comparable<T>> {
	...
	public void addAll(LinkedOrdList<T> mset) {
		LinkedOrdList<T> listaAux = new LinkedOrdList<>();
		int totalElems = this.size() + mset.size();
		int i = 0;
		while(i < totalElems) {
			if((!this.isEmpty()) && (!mset.isEmpty())) {
				T nodeL = this.removeFirst();
				T nodeR = mset.removeFirst();
				
				if(nodeL.compareTo(nodeR) < 0) {
					listaAux.add(nodeL);
					mset.add(nodeR);
				} else {
					listaAux.add(nodeR);
				...
				if(mset.isEmpty()) {
					while(!this.isEmpty()) {
						listaAux.add(this.removeFirst());
						i++;
					}
				} else {
					while(!mset.isEmpty()) {
						listaAux.add(mset.removeFirst());
						i++;
					}
				}
			}
		}
		this.contenido = listaAux.contenido;
		this.cardinal = totalElems;
	}
